...
- **`triage`** (LLM, `run_triage`): may only reorder findings *within* their severity tier and may request bounded CVE-detail escalations by `ref`/`cpe` (max `_MAX_ESCALATIONS` = 3, validated against the table). Cannot invent a finding or promote severity; a malformed/out-of-bounds response falls back to deterministic tier+CVSS ordering (`_fallback_order`).
...
**Windows prerequisites:** Npcap (for nmap LAN scans), Administrator (for elevation-gated audit checks and raw-socket nmap). Installer/bundling — a single signed package provisioning nmap+Npcap, nuclei.exe, and the Python runtime — is separate follow-on distribution work (see `GuideToWindowsCompatibility.txt`); on Windows, code-signing is close to mandatory since unsigned pentest bundles trip SmartScreen and Defender quarantine.
